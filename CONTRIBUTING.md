...
완료하고 검증한 단위로 커밋하고 바로 push합니다. 커밋 메시지에는 문제, 실제 변경, 실행한 검사와 남은 제한을 간결하게 기록합니다. 과거 단계를 소급한 가짜 커밋이나 단계별 Markdown 실행 일지를 만들지 않습니다. CI의 최종 `required` job과 저장소 branch protection 설정은 별개이며 후자는 적용 여부를 확인해야 합니다.
