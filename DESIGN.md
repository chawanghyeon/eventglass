...
### 0.2 Paths and source preservation

Implementation root: the repository root; module `github.com/chawanghyeon/eventglass`, binary `eventglass-go`. This is the only active product implementation. The final former implementation is recoverable from repository tag `rust-version`; it is not an active build or comparison dependency.
...
Record actual revisions, dirty state, toolchains, locks, and engine versions for every reproducibility claim.
...
The user required DuckDB2.0 whenever DuckDB is used and explicitly authorized the official prerelease after being shown that no stable2.0 release or matching duckdb-go bundle existed on2026-09-18. G00 therefore pins the official `v2.0-cyanoptera` extension-sync commit `800d6b748774cc958731a14df7b9cb170b50c075` (`v2.0.0-dev84020`) and source archive SHA-256 `226c9627c9c9379f6b1725e387339fd465e18c72a3dbeb6b65671eeb8cb29b5c`. This snapshot pins the matching official httpfs commit `ac145d8e83d1f3be97b978487cdf08c03f60df8c`; a newer branch head was rejected because its own pinned httpfs revision failed the Debian12/GCC12 build. The official github.com/duckdb/duckdb-go/v2 v2.10505.0 module is only the Go C-API adapter: every executable build MUST use `duckdb_use_static_lib` and link the pinned2.0 bundle; its bundled1.5.5 library must never execute. G00 revalidates Linux arm64/CGO/extension ABI and all engine contracts. An emulated amd64 build was deliberately stopped after the user narrowed the active platform to their ARM64 environment on2026-09-19; amd64 is neither verified nor claimed. Prerelease status remains an explicit release risk, not a stable2.0 claim. Start from the existing benchmark's Go1.26.5 and lock a verified patch.
...
Pin and maintain a MinIO compatibility fixture for local S3 contract tests. Do not recommend archived community distributions as a new production default. Garage is the initial self-host candidate, gated on S3 contracts/release provenance/restore and exact digest. If it fails, do not silently call an alternative production-ready. Release requires actual AWS plus at least one self-host backend.

Linux arm64 is the supported initial platform; amd64 remains explicitly unverified until separately requested. Pin CGO/native/extension ABI in images with SBOM/notices/checksums. No runtime package downloads. Non-root, readonly root filesystem, scoped scratch, localhost child gateway, private PG, narrow S3 prefix rights.
...
- S3 durable ACK p95<=500ms including network/batching.
...
## 21. Validation and cost contract

Mode A compares normalization/query output to independent oracles; mode B measures actual end-to-end durability and recovery.

Use fixed10k/100k/1m/10m seeds/SDK fixtures, IDs/times/sizes/cardinality/error ratio. Compare exact filters/time/count against an independent oracle. Include retries/duplicates/ID-less logs/equal timestamps/late events/null/missing/dotted paths/mixed numeric types.
...
Cost includes compute seconds, storage/journal/backup/index GB-month, S3 PUT/GET/LIST, transfer, PG/PITR, and self-host resources; record region/date/pricing parameters and do not invent monthly prices. Report revisions/dirty state/toolchains/locks/engine/exact semantics/dataset checksum/cgroup peak/OOM/latency percentiles/scans/network/PG bytes/completeness.
...
Deliver independent go.mod/go.sum, pinned toolchain/native/extensions/images, CLI skeleton, scripts/check, Compose PG+S3, fixture manifest, baseline hashes. Verify the driver on Linux arm64, nested attrs/DECIMAL roundtrip, memory/spill/cancel/kill, bound read_parquet lists, real gateway Range/block hashes, Zstd/row-group metadata, remote permissions, offline startup, migrations/rollback/checksums. Separate localhost from actual AWS validation. Failure requires library/API investigation, not hand-written codecs/parsers or invented performance. Do not infer amd64 support from source portability or a partial emulated build.
...
### G07 — Resources, scaling, and cost
...
Require actual AWS and S3-compatible smoke/restore, explicit SDK support matrix, reproducible arm64 image/SBOM, secret scans, migration/backward-schema reads/rollback compatibility, admin guide/sample DSN/retention/resources/license notices. The support matrix must list amd64 as unverified rather than imply portability evidence.
