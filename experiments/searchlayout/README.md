...
`go test ./experiments/searchlayout -run 'TestLiteratureAlternatives|TestTrigramCandidates|TestTopKPruningCannotProduceExactAggregates' -v -count=1` also probes fixed-width block postings, bit-sliced exact SUM, byte-trigram substring candidates with source recheck, and the Top-K/exact-aggregate conflict. Its [ARM64 raw log](evidence-2026-09-24/literature-alternatives-darwin-arm64.txt) and [design decision](../../docs/observe/search-engine-design.md) keep these component results separate from an end-to-end or real-S3 comparison. The `hybrid` experiment uses a plain dense bitset, **not** Roaring.

`go test ./experiments/searchlayout -run '^TestBlockDictionaryWithInlinePostings$' -v -count=1` checks a 512-term sorted block dictionary with inline postings for lists of six or fewer rows against the existing single-segment manifest layout. The [raw result](evidence-2026-09-24/dictionary-blocks-darwin-arm64.txt) counts catalog plus postings on both sides and validates every decoded term and inline list. It does not measure S3 reads or merges.

`go test ./experiments/searchlayout -run '^TestUniversalSelectionAndAggregation$' -bench '^BenchmarkUniversalSelection$' -benchtime=200ms -count=3` probes a shared sorted-document-ID result for indexed tokens/values, typed sparse columns, and exact source scans. The full-scan oracle checks 20 random seeds, 16 predicate shapes, four dynamic group paths, scope/live filters, exact counts/sums, and Top-K. Its [ARM64 component timings](evidence-2026-09-24/unified-selection-darwin-arm64.txt) do not include S3, compression, or service execution.
