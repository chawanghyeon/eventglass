...
// It needs no browser secret: the snapshot principal must still correspond to
// one live session, and every captured authority revision is rechecked under
// shared locks before a task result can become authoritative.
func lockDurableQueryScope(ctx context.Context, tx pgx.Tx, tenantID int64, snapshotID string, expectedGeneration int64) (durableQueryScope, error) {
	...
	var state string
	if err := tx.QueryRow(ctx, `SELECT snapshot_id::text,tenant_id,user_id,principal_ref,auth_revision,tenant_auth_revision,
		storage_generation,expires_at,max_until,state FROM query_snapshots
		WHERE tenant_id=$1 AND snapshot_id=$2 AND principal_kind='user'`, tenantID, snapshotID).Scan(
		&scope.SnapshotID, &scope.TenantID, &scope.UserID, &scope.PrincipalHash, &scope.AuthRevision,
	...
	if err := tx.QueryRow(ctx, `SELECT t.state,t.auth_revision,u.state,u.auth_revision,m.role
		...
		&tenantState, &tenantRevision, &userState, &userRevision, &role); err != nil {
		return scope, errors.Join(ErrForbidden, err)
	}
	if tenantState != "active" || userState != "active" || tenantRevision != scope.TenantRevision || userRevision != scope.AuthRevision {
	...
	var finalUser, finalAuth, finalTenantAuth, finalGeneration int64
	var finalExpires time.Time
	if err := tx.QueryRow(ctx, `SELECT user_id,principal_ref,auth_revision,tenant_auth_revision,storage_generation,expires_at,state
		FROM query_snapshots WHERE tenant_id=$1 AND snapshot_id=$2 FOR SHARE`, tenantID, snapshotID).Scan(
		&finalUser, &finalPrincipal, &finalAuth, &finalTenantAuth, &finalGeneration, &finalExpires, &finalState); err != nil {
		return scope, err
	}
	if finalState != "active" || !finalExpires.After(now) || finalUser != scope.UserID || finalPrincipal != scope.PrincipalHash || finalAuth != scope.AuthRevision || finalTenantAuth != scope.TenantRevision || finalGeneration != expectedGeneration {
		return scope, ErrForbidden
	}
	return scope, nil
}
