...
func (service *PublicQueryService) Live(ctx context.Context, principal control.SessionPrincipal, tokenHash [32]byte, request query.PublicLiveRequest, resume string, emit func(query.LiveEvent) error) error {
	if err := service.validate(); err != nil {
		return err
	}
	if service.Sync == nil || emit == nil {
		return errors.New("live query execution is unavailable")
	}
	if err := validateLiveRequest(request, time.Now()); err != nil {
		return errors.Join(api.ErrPublicQueryInvalid, err)
	}
	...
	windowStart, windowRows := time.Now(), 0
	...
	for {
		...
		case now := <-ticker.C:
			if now.Sub(windowStart) >= 10*time.Second {
				windowStart, windowRows = now, 0
			}
			before := positions
			page, cuts, snapshotID, pollErr := service.pollLive(ctx, tokenHash, request, positions, now)
			if pollErr != nil {
			...
			if windowRows+len(page.Rows) > query.LiveMaximumRows {
				_ = service.Control.ReleaseSnapshot(context.WithoutCancel(ctx), tokenHash, request.TenantID, snapshotID)
				return emit(query.LiveEvent{Type: "resync_required", Data: liveControlData{Code: "rate_limit", Retryable: false}})
			}
			...
			if len(page.Rows) > 0 {
				token, signErr := service.signLiveCheckpoint(principalHash, scopeHash, positions, now)
				if signErr != nil {
					_ = service.Control.ReleaseSnapshot(context.WithoutCancel(ctx), tokenHash, request.TenantID, snapshotID)
					return signErr
				}
...
func (service *PublicQueryService) livePreflight(ctx context.Context, tokenHash [32]byte, request query.PublicLiveRequest, resume, scopeHash, principalHash string, emit func(query.LiveEvent) error) ([model.LaneCount]query.LivePosition, bool, bool, error) {
	now := time.Now()
	...
	token, err := service.signLiveCheckpoint(principalHash, scopeHash, positions, now)
	if err == nil {
...
func (service *PublicQueryService) pollLive(ctx context.Context, tokenHash [32]byte, request query.PublicLiveRequest, positions [model.LaneCount]query.LivePosition, now time.Time) (page livePage, cuts [model.LaneCount]int64, snapshotID string, resultErr error) {
	snapshot, err := service.createLiveSnapshot(ctx, tokenHash, request, now)
	if err != nil {
	...
	cuts = liveSnapshotCuts(snapshot)
	plan, err := query.BuildPlan(liveDataset(request, now).Spec, snapshotScope(snapshot), request.Filter)
	if err != nil {
		return page, cuts, snapshotID, errors.Join(api.ErrPublicQueryInvalid, err)
	}
	operation, err := query.BuildLiveOperation(plan, positions)
	if err != nil {
		return page, cuts, snapshotID, err
	}
	job, synchronous, err := service.createAndPlan(ctx, tokenHash, snapshot, "search", operation, query.ModeSync)
	if err != nil {
		return page, cuts, snapshotID, err
	}
	if !synchronous {
...
func (service *PublicQueryService) createLiveSnapshot(ctx context.Context, tokenHash [32]byte, request query.PublicLiveRequest, now time.Time) (model.QuerySnapshot, error) {
...
func liveDataset(request query.PublicLiveRequest, now time.Time) query.PublicDataset {
	start := now.Add(-liveLookback).UnixMicro()
	if request.CatchupStart != nil && request.CatchupStart.UnixMicro() > start {
		start = request.CatchupStart.UnixMicro()
	}
	spec := model.DatasetSpec{TenantID: request.TenantID, ProjectIDs: append([]int64(nil), request.ProjectIDs...), Kinds: append([]model.Kind(nil), request.Kinds...), TimeBasis: model.QueryTimeReceived, StartUS: start, EndUS: now.Add(time.Second).UnixMicro(), Filter: append([]byte(nil), request.Canonical...)}
...
func (service *PublicQueryService) executeLiveQuery(ctx context.Context, tokenHash [32]byte, tenantID int64, queryID string) (control.QueryStatus, error) {
...
func (service *PublicQueryService) finalizeLiveResult(ctx context.Context, status control.QueryStatus) (livePage, error) {
	if status.Result == nil {
		return livePage{}, errors.New("live result artifact is unavailable")
	}
	var operation engine.QueryOperation
	if err := strictAppJSON(status.Operation, &operation); err != nil || operation.Version != engine.QueryExecutionProtocolVersion || operation.Kind != "live" || operation.Result.Kind != "live" || operation.Result.Limit != query.LivePageRows {
...
func (service *PublicQueryService) signLiveCheckpoint(principalHash, scopeHash string, positions [model.LaneCount]query.LivePosition, now time.Time) (string, error) {
	return service.Tokens.SignLive(service.StorageGeneration, principalHash, scopeHash, positions, now.Add(liveTokenLifetime).UnixMicro())
}
...
func emitBoundedLive(emit func(query.LiveEvent) error, event query.LiveEvent) error {
	...
	if len(encoded) > query.LiveMaximumPending {
