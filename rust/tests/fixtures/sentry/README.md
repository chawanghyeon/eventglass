...
Run `tools/sdk-fixtures/bootstrap-live.sh` from the repository root to install the exact Python lock and Node package-lock into ignored task-local directories and verify their versions. Run `tools/sdk-fixtures/run-live.sh` for the explicit localhost gate; it selects the crate under `rust/` and Cargo supplies the built executable (normally `rust/target/debug/eventglass`, or the configured CARGO_TARGET_DIR). It creates marked temporary data directories, configures projects through the real admin API, checks observer-captured 202 receipts, restarts the first process, inspects published native documents, and repeats the Node case directly against a second Eventglass process without the observer. Missing or mismatched SDK environments fail this gate; they are never silently skipped.
