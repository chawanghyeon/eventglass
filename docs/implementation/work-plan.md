...
| R4 / R3 — external release gate open | deploy backend locks; release verification; operator/upgrade guides; SBOM/notices | ARM64 `./scripts/check recovery` now supports both disposable MinIO and an explicitly scoped AWS scratch-bucket prefix; the fresh local PostgreSQL base/WAL+S3 restore and missing-object fail-closed path pass. AWS mode is implemented but not exercised: this host has no AWS CLI or identity. Fresh ARM64 browser E2E, the documented pinned live SDK matrix, and changed-file secret hook pass. Signed release provenance, image/dependency security and license scans, and deployment rollback rehearsal remain unverified. **G08 remains incomplete; do not advertise a release.** |
