...
| R4 / R3 — external release gate open | deploy backend locks; `scripts/check-release`; `.github/workflows/release-evidence.yml`; operator/upgrade guides | ARM64 `./scripts/check recovery` supports disposable MinIO and an explicitly scoped AWS scratch-bucket prefix; fresh local PostgreSQL base/WAL+S3 restore and missing-object fail-closed pass. `./scripts/check release` and fresh `./scripts/check-browser` passed on Colima ARM64 from the `8401930` working tree: non-root/read-only image smoke, `linux/arm64` identity, checksummed archive, and browser E2E. Workflow/shell lint and Go unit/layout checks pass. A main-only manual workflow is implemented for browser E2E, AWS scratch recovery, SPDX, Trivy, Go license notices, and signed provenance/SBOM, but has not run. AWS mode is still unverified on this host without AWS CLI/identity; rollback rehearsal and a proven self-host S3-compatible provider also remain open. **G08 remains incomplete; do not advertise a release.** |
