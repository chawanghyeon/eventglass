...
| R4 / R3 — external release gate open | deploy backend locks; `deploy/garage`; `scripts/check-release`; `.github/workflows/release-evidence.yml`; operator/upgrade guides | ARM64 `./scripts/check recovery` supports disposable MinIO and an explicitly scoped AWS scratch-bucket prefix. `./scripts/check garage` passes pinned Garage S3 Range/list/multipart/auth and durable-ingest contracts plus real PostgreSQL base/WAL+S3 restore, activated-generation old-object read, non-adoption of newer objects, and missing-object fail-closed. The latest clean `f03b800` source passed `./scripts/check release` on Colima ARM64: `linux/arm64`, non-root/read-only smoke, version check and 60MiB checksummed archive. Browser E2E passed on ARM64 from the earlier `8401930` working tree. A main-only manual workflow is implemented for browser E2E, Garage and AWS recovery, SPDX, Trivy, Go license notices, and signed provenance/SBOM, but has not run. AWS mode is still unverified on this host without AWS CLI/identity; rollback rehearsal and workflow scan/attestation evidence remain open. **G08 remains incomplete; do not advertise a release.** |
