...
| R4 / R3 — external release gate open | deploy backend locks; `deploy/garage`; `scripts/check-release`; `.github/workflows/release-evidence.yml`; operator/upgrade guides | ARM64 `./scripts/check recovery` supports disposable MinIO and an explicitly scoped AWS scratch-bucket prefix. `./scripts/check garage` passes pinned Garage S3 Range/list/multipart/auth and durable-ingest contracts plus real PostgreSQL base/WAL+S3 restore, activated-generation old-object read, non-adoption of newer objects, and missing-object fail-closed. Go build/runtime APT repositories are locked to Debian snapshot20260926; the linked DuckDB2.0 bundle must match its pinned SHA-256, without changing the native-cache recipe. Clean source`9c50906` passed `./scripts/check release` and `./scripts/check-browser`; Syft1.42.3 produced an SPDX SBOM (147 packages) from the exact image and go-licenses2.0.1 reported/saved49 third-party license notices for the production binary. Trivy0.74.0 scanned that same clean archive and still fails the HIGH/CRITICAL gate with0 critical and47 high Debian13 package findings; do not suppress them. The main-only workflow's license command now targets the shipped binary and ignores only Eventglass's own module. AWS mode remains unverified without AWS CLI/identity; rollback rehearsal, AWS evidence and signed attestations remain open. **G08 remains incomplete; do not advertise a release.** |
