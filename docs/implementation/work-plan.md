...
| R4 / R3 — external release gate open | deploy backend locks; `deploy/garage`; `scripts/check-release`; `.github/workflows/release-evidence.yml`; operator/upgrade guides | ARM64 `./scripts/check recovery` supports disposable MinIO and an explicitly scoped AWS scratch-bucket prefix. `./scripts/check garage` passes pinned Garage S3 Range/list/multipart/auth and durable-ingest contracts plus real PostgreSQL base/WAL+S3 restore, activated-generation old-object read, non-adoption of newer objects, and missing-object fail-closed. The runtime is now pinned to Debian13 ARM64 and `pgx`/`x/crypto` were updated to scanner-reported fixed releases. The rebuilt image passed `./scripts/check release` and `./scripts/check-browser`; Syft1.42.3 produced an SPDX SBOM (147 packages), and go-licenses2.0.1 reported/saved49 third-party license notices for the production binary. Trivy0.74.0 still fails the HIGH/CRITICAL gate with0 critical and47 high Debian package findings (no fixed versions in its current database); do not suppress them. The main-only workflow's license command now targets the shipped binary and ignores only Eventglass's own module. AWS mode remains unverified without AWS CLI/identity; rollback rehearsal, AWS evidence and signed attestations remain open. **G08 remains incomplete; do not advertise a release.** |
