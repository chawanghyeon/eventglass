...
| 단일 객체 색인+중복 제거 원문 페이지 | 실제 StageRecord 기본/무작위 토큰·속성값 각 10만 건을 전건 왕복 복원. 현행 두 Parquet 대비 합계 8.8%/20.2% 감소. 무작위 입력에서 MinIO 상세 조회 30회×3 실행 | 이 입력의 저장량·상세 정확성 통과. 작은 객체 5 GET의 블록 중복 읽기, 전체 제품 타입·PG/S3 수명주기 미검증 |
