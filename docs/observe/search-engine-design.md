...
| 실제 StageRecord 압축 색인의 다중 세그먼트 | 고유 토큰·속성값 10만 건의 4→1 세그먼트와 이전 로컬 세대에서 6개 집계 질의를 독립 oracle와 비교. MinIO 3질의×30회에서 분할 4 GET, 병합 1 GET | 색인 부분의 정확성·MinIO 요청 수 통과. PG/S3 세대 공개·원문 병합·실제 AWS 비용은 미검증 |
| 단일 객체 색인+중복 제거 원문 페이지 | 실제 StageRecord 기본/무작위 토큰·속성값 각 10만 건을 전건 왕복 복원. 현행 두 Parquet 대비 합계 8.8%/20.2% 감소. MinIO 상세는 고정 tail Range로 3 GET | 저장량·상세 정확성 통과. 기본 입력의 상세 S3 GET/바이트는 현행 payload보다 큼; 전체 제품 타입·PG/S3 수명주기 미검증 |
...
[동일 압축 색인의 분할·병합 검사](../../experiments/searchlayout/product_compact_generations_test.go)는 앞의 `StageRecord` 입력과 같은 색인 생성 함수를 사용했다. 무작위 128비트 고유 토큰·무작위 속성값 10만 건을 4개 세그먼트로 나눈 뒤 단일 병합 세그먼트와 비교했다. `fatal`, `request`, `request AND latency>=500`, `request AND custom/236 EXISTS`, 고유 `trace`, 부재어의 그룹별 정확한 건수·severity 합이 입력 레코드를 직접 읽는 독립 oracle와 일치했다. 한 세그먼트의 검색값을 바꿔 새 세대를 만든 후 이전 세그먼트를 다시 읽어 이전 결과가 유지되는 것도 확인했다. [ARM64 CPU1/512MiB 원시 로그](../../experiments/searchlayout/evidence-2026-09-24/actual-product-compact-generations-random-128-attr-100k.txt)에서 4개 색인 객체 합계는 4,603,914B, 병합 색인 하나는 4,612,212B였고 OOM은 0이었다. cgroup peak는 536,875,008B로 한계에 닿아 메모리 여유를 입증하지 못한다. 이 검사는 **색인 부분의 로컬 논리적 세대**만 다룬다. 원문 Parquet의 병합, PG 카탈로그 원자 교체·권한·ACK, S3에서 고정된 객체 키 읽기와 삭제 지연은 검증하지 않았다.

[격리 MinIO의 분할·병합 Range 실행](../../experiments/searchlayout/evidence-2026-09-24/actual-product-compact-generations-minio-random-128-attr-100k.txt)과 [독립 반복](../../experiments/searchlayout/evidence-2026-09-24/actual-product-compact-generations-minio-random-128-attr-100k-repeat.txt)은 위 10만 건의 색인 5개를 매번 실제 `S3Store.PutStream`으로 올렸다. 각 실행에서 5 PUT·5 HEAD·5 업로드 검증 GET 뒤 `request`, `request AND latency>=500`, 고유 `trace`를 분할·병합 교차 순서로 각 30회 실행하고 매번 독립 oracle와 대조했다. 실행당 450 Range GET/829,451,340B였으며, 질의당 분할은 **4 GET/4,603,914B**, 병합은 **1 GET/4,612,212B**였다. `request AND latency>=500` p95 분할/병합은 첫 실행 **216.78/196.12ms**, 반복은 **92.66/101.98ms**로 크게 흔들렸다. 후보의 전체 압축 어휘를 각 세그먼트에서 다시 읽는 구조이므로 분할 상태가 요청 수를 늘린다는 점은 확인됐지만, localhost MinIO의 지연으로 AWS p95/p99나 총비용 우열을 결정할 수 없다. 두 실행은 OOM 0, cgroup peak 428,052,480/440,369,152B였고 PG 공개 세대·권한·원문 병합은 여전히 범위 밖이다.

위 후보의 +40% 저장량에는 원문 Parquet에 그대로 남은 `attrs`·`project_id`·`service`·`severity_number`의 중복이 포함된다. [중복 제거 원문 페이지 실험](../../experiments/searchlayout/product_compact_residual_test.go)은 이 네 필드만 원문 저장부에서 빼고, **`search_values`와 나머지 StageRecord 전체는 보존**했다. 128문서별 Zstd 페이지와 실제 offset/length 디렉터리로 저장한 뒤 색인 코어와 합쳐 모든 레코드를 원래 StageRecord와 `DeepEqual`로 확인했다. [기본 10만](../../experiments/searchlayout/evidence-2026-09-24/actual-product-compact-residual-baseline-100k.txt)은 원문 2,064,297B+색인 47,246B=**2,111,543B** 대 현행 두 Parquet **2,315,870B**(−8.8%), [무작위 128비트 토큰·속성값 10만](../../experiments/searchlayout/evidence-2026-09-24/actual-product-compact-residual-random-128-attr-100k.txt)은 원문 4,387,801B+색인 4,612,212B=**9,000,013B** 대 **11,273,200B**(−20.2%)였다. [같은 무작위 입력 1만](../../experiments/searchlayout/evidence-2026-09-24/actual-product-compact-residual-random-128-attr-10k.txt)도 −22.4%였다. 이 단계는 두 조각의 저장량과 복원만 확인했고, 아래 시험에서 단일 S3 객체의 원격 상세를 별도로 측정했다. CPU1/512MiB에서 OOM은 없었으나 두 10만 실행 모두 cgroup peak가 한계에 닿았다. 이 수치로 쓰기·병합·백업·실제 AWS 비용까지 우수하다고 판정하지 않는다.

후속 [단일 객체 Range 시험](../../experiments/searchlayout/product_compact_compound_minio_test.go)은 앞의 색인과 원문 페이지를 footer 32B와 함께 한 객체로 묶었다. **1만 건 객체 899,723B**, **10만 무작위 객체 9,000,045B**로 현행 두 Parquet 대비 각각 −22.4%/−20.2%다. 실제 `S3Store.PutStream`과 해시 검증 게이트웨이를 거쳐 대상 레코드의 색인 코어·원문 페이지를 Range로 읽고 전체 StageRecord를 복원했다. 비교 대상인 현행 payload Parquet도 원문 JSON과 `canonical_metadata_json`을 합쳐 원래 `Record`와 일치시켰다. [1만 최종 로그](../../experiments/searchlayout/evidence-2026-09-24/actual-product-compact-compound-minio-random-128-attr-10k.txt)에서는 후보가 상세 조회당 **5 GET/4,498,615B**, payload가 **1 GET/671,767B**였다. 객체가 1MiB 미만이어서 현재 게이트웨이의 1MiB 검증 블록을 5번 다시 가져온 비용이다. [10만 첫 실행](../../experiments/searchlayout/evidence-2026-09-24/actual-product-compact-compound-minio-random-128-attr-100k.txt)과 [반복](../../experiments/searchlayout/evidence-2026-09-24/actual-product-compact-compound-minio-random-128-attr-100k-repeat.txt), [전체 메타데이터 검사 최종](../../experiments/searchlayout/evidence-2026-09-24/actual-product-compact-compound-minio-random-128-attr-100k-final.txt)에서는 후보 **5 GET/6,028,615B**, payload **4 GET/6,533,456B**였다. 후보/payload p95는 각각 **33.74/50.33**, **33.91/59.10**, **37.30/62.44ms**였다. 후보는 테스트의 Go 복원기, payload는 질의마다 새로 연 DuckDB이므로 실행기 자체의 CPU 차이를 포함한다. 현재 후보는 원문 상세에서도 GET 수가 더 많고 512MiB cgroup peak에 닿는다. localhost MinIO 결과를 실제 AWS·제품 p95/p99 우위로 확대하지 않으며, GET을 줄일 물리 디렉터리 배치와 전체 제품 경로가 남는다.

단일 객체 끝의 페이지 디렉터리 크기는 공개 문서 수와 128문서 페이지 크기로 정해진다. 이를 이용해 **디렉터리·원문 footer·객체 footer를 하나의 고정 tail Range**로 읽고, 색인 코어와 해당 원문 페이지만 각각 한 Range로 읽도록 바꿨다. 질의 종류나 관측 속도에 따라 다른 물리 경로를 고르는 정책은 없다. [무작위 10만 첫 실행](../../experiments/searchlayout/evidence-2026-09-24/actual-product-compact-compound-tail-minio-random-128-attr-100k.txt)과 [반복](../../experiments/searchlayout/evidence-2026-09-24/actual-product-compact-compound-tail-minio-random-128-attr-100k-repeat.txt)의 후보는 **3 GET/4,805,741B**, 현행 payload는 **4 GET/6,533,456B**였고 p95 후보/payload는 **31.38/47.05**, **46.34/72.08ms**였다. [무작위 1만](../../experiments/searchlayout/evidence-2026-09-24/actual-product-compact-compound-tail-minio-random-128-attr-10k.txt)은 여전히 3 GET/2,699,169B 대 1 GET/671,767B였다. [기본 10만](../../experiments/searchlayout/evidence-2026-09-24/actual-product-compact-compound-tail-minio-baseline-100k.txt)도 후보 **3 GET/2,111,575B**, payload **1 GET/1,305,023B**로 요청·바이트가 역전된다. 두 입력 모두 원문·전체 메타데이터가 일치했고 OOM은 없었지만 10만 실행은 cgroup peak가 512MiB 한계에 닿았다. 따라서 단일 객체는 저장량과 고유값 입력의 상세 읽기에서 가능성을 보였으나, 모든 입력·질의의 비용 우위를 입증하지 못한다.

후속 [무작위 10만](../../experiments/searchlayout/evidence-2026-09-24/actual-product-compact-compound-tail-minio-random-128-attr-100k-assert.txt)·[기본 10만](../../experiments/searchlayout/evidence-2026-09-24/actual-product-compact-compound-tail-minio-baseline-100k-assert.txt)은 30회 각각의 후보 조회가 정확히 **3 GET·동일 바이트**인지 테스트에서 강제한 재실행이다. 두 입력 모두 통과했고 원문·메타데이터 정답도 유지됐다. 이는 MinIO 검증 게이트웨이의 고정 블록 정책 아래에서의 요청 수 증거다. 원격 왕복 지연이 다른 실제 AWS나 캐시가 있는 제품 worker의 요청 수를 자동으로 보장하지는 않는다.
