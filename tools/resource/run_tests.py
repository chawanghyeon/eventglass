...
report = {'limits': limits, 'expected_memory_bytes': limit, 'tests': [], 'passed': False}
def save():
...
try:
    ...
    for artifact in manifest:
        ...
        listing = subprocess.run([executable, '--list', '--format', 'terse'], text=True, capture_output=True, timeout=60, check=True)
        count = sum(line.endswith(': test') for line in listing.stdout.splitlines())
        assert count > 0, name + ' selected zero tests'
        started = time.monotonic()
        result = subprocess.run([executable, '--test-threads=1'], text=True, capture_output=True, timeout=300)
