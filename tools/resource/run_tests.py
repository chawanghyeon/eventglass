...
report = {'limits': limits, 'expected_memory_bytes': limit, 'expected_cpus': cpus, 'test_uid': 65534, 'tests': [], 'passed': False}
def save():
...
try:
    ...
    assert quota == int(period * cpus), limits
    assert limits['memory.max'] == str(limit) and limits['memory.swap.max'] == '0', limits
    ...
    print('Verified cgroup: CPU quota ' + str(cpus) + ', memory ' + str(limit) + ', swap 0', flush=True)
