...
Current status: G00 engine/storage/isolation contracts, G01 SDK fixture/normalization contracts, all G02 packets I1–I5 (durable ingestion, bounded runtime, lease fencing, and the Linux ARM64 crash/resource gate), all G03 packets P1–P4 (publication catalog, isolated paired-Parquet conversion, fenced Prepare/Publish workers, Issue lifecycle, and publication crash evidence), and G04 packet Q1 (recoverable setup, browser sessions, scoped authorization, project keys, and generated v1 wire contracts) are complete. Q2 is the first incomplete packet; G04–G08 remain incomplete overall, so the repository does not yet advertise a deployable Go release. See [`DESIGN.md`](DESIGN.md), [`SDK-SUPPORT.md`](SDK-SUPPORT.md), and [`CONTRIBUTING.md`](CONTRIBUTING.md) for the normative contract, verified SDK scope, and commands.
...
lifecycle, and deterministic OpenAPI generation. Query execution, operator
...
design contracts, not evidence that Q2–G08 have been implemented or verified.
