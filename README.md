...
Current status: G00 engine/storage/isolation contracts, G01 SDK fixture/normalization contracts, all G02 packets I1–I5 (durable ingestion, bounded runtime, lease fencing, and the Linux ARM64 crash/resource gate), all G03 packets P1–P4 (publication catalog, isolated paired-Parquet conversion, fenced Prepare/Publish workers, Issue lifecycle, and publication crash evidence), and G04 packets Q1–Q3 (recoverable setup and auth, generated v1 wire contracts, typed CEL/JSON filters, canonical dataset identity, scope-safe DuckDB lowering, authorized snapshots, and signed read/cursor tokens) are complete. Q4 is the first incomplete packet; G04–G08 remain incomplete overall, so the repository does not yet advertise a deployable Go release. See [`DESIGN.md`](DESIGN.md), [`SDK-SUPPORT.md`](SDK-SUPPORT.md), and [`CONTRIBUTING.md`](CONTRIBUTING.md) for the normative contract, verified SDK scope, and commands.
...
and purpose-separated signed read/cursor tokens. Query execution, operator
surfaces, and backup/PITR recovery remain later gates.
...
design contracts, not evidence that Q4–G08 have been implemented or verified.
