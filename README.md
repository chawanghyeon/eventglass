...
로컬 Linux 자원 검증 기준은 1 CPU와 1 GiB RAM, swap 없음이며 `./scripts/check-resource`로 실행합니다. 현재 운영 서버는 단독 사용을 전제로 CPU 0.25코어, RAM 1 GiB, swap 없음으로 제한합니다. `./scripts/check-resource --cpus 0.25`와 `./scripts/check-benchmark 100k --operational`은 동일한 운영 CPU·메모리 조건을 검증하며, 후자는 data_dir 사용량을 50GB 예산과 대조합니다. 50GB 실제 볼륨 quota, HTTP wire ACK, S3 비용은 이 benchmark의 측정 범위 밖입니다. GitHub CI는 로컬에서 검증한 바이너리의 배포 활성화만 수행합니다. 100K Record PR benchmark는 1 CPU와 더 작은 512 MiB 한도에서 실행합니다. 용량과 지연은 데이터 분포·샤드 수·검색 범위에 따라 달라지므로 배포 전 실제 트래픽과 보존 기간으로 측정해야 합니다.
