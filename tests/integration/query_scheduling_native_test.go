...
func TestPublicQueryEndToEndTenantTurnsActualWorker(t *testing.T) {
	...
	defer cancel()
	second := addSchedulingTenant(t, ctx, fixture, 1812)
	...
	if _, err := fixture.pool.Exec(ctx, `CREATE TABLE test_query_claim_order (
		ordinal BIGSERIAL PRIMARY KEY CHECK(ordinal<=16),tenant_id BIGINT NOT NULL,query_id UUID NOT NULL);
		CREATE FUNCTION test_record_query_claim() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN
		  IF NEW.state='running' AND NEW.attempt>OLD.attempt THEN
		    INSERT INTO test_query_claim_order(tenant_id,query_id) VALUES(NEW.tenant_id,NEW.query_id);
		  END IF;
		  RETURN NEW;
		END $$;
	...
	worker.stop()
	var turns []int64
	if err := fixture.pool.QueryRow(ctx, `SELECT array_agg(tenant_id ORDER BY ordinal) FROM test_query_claim_order`).Scan(&turns); err != nil {
		t.Fatal(err)
	}
	t.Logf("actual single-worker query tenant order=%v", turns)
	for _, request := range pending {
	...
	if !slices.Equal(turns, want) {
		t.Fatal(fmt.Sprintf("older tenant hid ready search: turns=%v expected=%v", turns, want))
	}
}
